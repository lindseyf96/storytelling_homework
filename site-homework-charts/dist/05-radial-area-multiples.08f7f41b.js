...
var cities = ['NYC', 'Tuscon', 'Lima', 'Beijing', 'Stockholm', 'Melbourne'];
var radius = 60;
var radiusScale = d3.scaleLinear().domain([0, 90]).range([0, radius]);
var angleScale = d3.scaleBand().domain(months).range([0, Math.PI * 2]);
var line = d3.radialArea().innerRadius(function (d) {
...
var bands = [20, 40, 60, 80, 100];
var labels = [20, 60, 100];
d3.csv(require('/data/all-temps.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  container.selectAll('.graph').data(nested).enter().append('g').attr('transform', function (d) {
    ...
    g.append('path').datum(datapoints).attr('d', function (d) {
      return line(d);
    }).attr('fill', 'pink').attr('opacity', 0.8).attr('stroke', 'none');
    g.selectAll('.scale-band').data(bands).enter().append('circle').attr('r', function (d) {
      return radiusScale(d);
    }).attr('fill', 'none').attr('stroke', 'lightgrey').attr('cx', 0).attr('cy', 0).lower();
    g.selectAll('.temp-label').data(labels).enter().append('text').text(function (d) {
      return d + '°';
    }).attr('text-anchor', 'middle').attr('x', 0).attr('y', function (d) {
      return -radiusScale(d);
    }).attr('dy', -1).attr('font-size', 11);
    g.append('text').text(function (d) {
      return d.key;
    }).attr('x', xPositionScale(d.cities)).attr('y', height / 3).attr('font-size', 15).attr('fill', 'black').attr('text-anchor', 'middle');
  });
}
},{"d3":"../node_modules/d3/index.js","/data/all-temps.csv":"data/all-temps.csv"}]},{},["scripts/05-radial-area-multiples.js"], null)
//# sourceMappingURL=/05-radial-area-multiples.08f7f41b.js.map