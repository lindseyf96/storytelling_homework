...
var svg = d3.select('#line-chart').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
d3.csv(require('../data/AAPL.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.append('path').datum(datapoints).attr('d', line).attr('stroke', '#4cc1fc').attr('stroke-width', 2).attr('fill', 'none');
  svg.append('text').text('AAPL stock price').attr('x', width / 2).attr('y', 0).attr('text-anchor', 'middle').attr('font-size', 22).attr('font-weight', 'bold');
  ...
  var yAxis = d3.axisLeft(yPositionScale).tickValues([100, 110, 120]).tickSize(-width);
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 3).attr('cx', function (d) {
    ...
    return yPositionScale(d.Close);
  }).attr('fill', '#50c1fb').on('mouseover', function (d, i) {
    console.log('mouseover on', this);
    d3.select(this).transition().duration(100).attr('r', 7);
    d3.select('.col-3').append('p').text('Date: ').style('font-weight', 'bold').append('tspan').text(d.Date).style('font-weight', 'normal').append('p').text('Close: ').style('font-weight', 'bold').append('tspan').text(d.Close).style('font-weight', 'normal');
  }).on('mouseout', function (d, i) {
    console.log('mouseout', this);
    d3.select(this).transition().duration(100).attr('r', 3);
    d3.select('.col-3').selectAll('p').remove();
  });
}
},{"d3":"../node_modules/d3/index.js","../data/AAPL.csv":"data/AAPL.csv"}]},{},["scripts/02-line-chart.js"], null)
//# sourceMappingURL=/02-line-chart.6bf8653e.js.map