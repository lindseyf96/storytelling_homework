...
var svg = d3.select('#chart-3').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
d3.csv(require('/data/ny-temps.csv')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});
var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec']; // I give you a month
// you give me back a number of radians

var angleScale = d3 // .scalePoint()
// .padding(0.5)
.scaleBand().domain(months).range([0, Math.PI * 2]);
var radius = 150;
var colorScale = d3.scaleOrdinal().range(['#B6D5E3', '#BCD3E1', '#C9D0DD', '#DACBD7', '#E9C6D2', '#F4C3CE', '#F4BFCB', '#F4C0CB', '#F0C5D0', '#DFCAD6', '#CFCEDB', '#BED3E0']); // If I sell 0 houses, I have a radius of 0
// If I sell 70 houses, I have a radius of... radius? 150

var radiusScale = d3.scaleLinear().domain([0, 75]).range([0, radius]); // Make the outside of the shape based on
...
var arc = d3.arc().innerRadius(0).outerRadius(function (d) {
...
d3.csv(require('/data/ny-temps.csv')).then(ready).catch(function (err) {
  return console.log('Failed with', err);
});

function ready(datapoints) {
  // Throw January onto the end so it connects
  datapoints.push(datapoints[0]); // Add a path for
  // EVERY!
  // SINGLE!
  //  DATAPOINT!

  svg.selectAll('.polar-bar').data(datapoints).enter().append('path').attr('d', arc).attr('fill', function (d) {
    return colorScale(d.month_name);
  });
  svg.append('circle').attr('r', 2).attr('cx', 0).attr('cy', 0); // No matter what, this is at 70 houses sold
  // svg
  //  .append('circle')
  //  .attr('cx', 0)
  //  .attr('cy', 0)
  ...
  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', function (d) {
    console.log(d);
...
},{"d3":"../node_modules/d3/index.js","/data/ny-temps.csv":"data/ny-temps.csv"}]},{},["scripts/03-wedges.js"], null)
//# sourceMappingURL=/03-wedges.a03241c5.js.map