...
var svg = d3.select('#chart-2').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // At the very least you'll need scales, and
...
var colorScale = d3.scaleOrdinal().range(['#90EE90', '#b19cd9', '#ffb347']);
var xPositionScale = d3.scalePoint().range([0, width]);
var tasks = ['Typing code', 'Rewriting code', 'Reading StackOverflow'];
var angleScale = d3.scaleBand().domain(tasks).range([0, Math.PI * 2]); // Read in the data

d3.csv(require('/data/time-breakdown-all.csv')).then(ready).catch(function (err) {
  return console.log('Failed with', err);
});

function ready(datapoints) {
  var projects = datapoints.map(function (d) {
    return d.project;
  });
  ...
  svg.selectAll('.graph').data(nested).enter().append('g').attr('transform', function (d) {
    console.log(d);
    ...
    container.selectAll('path').data(pie(datapoints)).enter().append('path').attr('d', function (d) {
      return arc(d);
    ...
    container.append('text').text(function (d) {
      return d.key;
    }).attr('x', xPositionScale(d.projects)).attr('y', height / 3).attr('font-size', 15).attr('fill', 'black').attr('text-anchor', 'middle');
  });
}
},{"d3":"../node_modules/d3/index.js","/data/time-breakdown-all.csv":"data/time-breakdown-all.csv"}]},{},["scripts/02-pie-multiples.js"], null)
//# sourceMappingURL=/02-pie-multiples.d4544f2a.js.map