...
var container = d3.select('#chart-06'); // Create your scales

var xPositionScale = d3.scaleLinear().domain([12, 55]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 0.3]).range([height, 0]); // Create a d3.line function that uses your scales

var jpArea = d3.line().x(function (d) {
  return xPositionScale(d.Age);
}).y(function (d) {
  return yPositionScale(d.ASFR_jp);
});
var usArea = d3.line().x(function (d) {
  return xPositionScale(d.Age);
}).y(function (d) {
  return yPositionScale(d.ASFR_us);
});
d3.csv(require('../data/fertility.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  var nested = d3.nest().key(function (d) {
    return d.Year;
  }).entries(datapoints); // Draw your dots
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('text').text(d.key).attr('x', width / 2).attr('y', -10).attr('text-anchor', 'middle').attr('font-size', '12');
    var usValues = d.values.map(function (d) {
      return d.ASFR_us;
    });
    var usSum = d3.sum(usValues).toFixed(2);
    svg.append('text').text(usSum).attr('x', width * 0.8).attr('y', 20).attr('text-anchor', 'middle').attr('font-size', '10').attr('fill', 'cyan');
    var jpValues = d.values.map(function (d) {
      return d.ASFR_jp;
    });
    var jpSum = d3.sum(jpValues).toFixed(2);
    svg.append('text').text(jpSum).attr('x', width * 0.8).attr('y', 32).attr('text-anchor', 'middle').attr('font-size', '10').attr('fill', 'red');
    svg.append('path').attr('d', usArea(d.values)).attr('fill', 'cyan').attr('opacity', 0.5);
    svg.append('path').attr('d', jpArea(d.values)).attr('fill', 'red').attr('opacity', 0.5);
...
},{"d3":"../node_modules/d3/index.js","../data/fertility.csv":"data/fertility.csv"}]},{},["scripts/chart-06.js"], null)
//# sourceMappingURL=/chart-06.1ed21415.js.map