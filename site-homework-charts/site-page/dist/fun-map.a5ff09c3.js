...
},{}],"data/world-cities.csv":[function(require,module,exports) {
module.exports = "/world-cities.10d1e8c8.csv";
},{}],"scripts/fun-map.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#fun-map').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var projection = d3.geoMercator();
var graticule = d3.geoGraticule(); // out geoPath needs a PROJECTION variable

var path = d3.geoPath().projection(projection);
var colorScale = d3.scaleSequential(d3.interpolateCool).domain([0, 500000]).clamp(true);
Promise.all([d3.json(require('/data/world.topojson')), d3.csv(require('/data/world-cities.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  svg.append('path').datum(graticule()).attr('d', path).attr('stroke', 'gray').attr('stroke-width', 0.5);
  svg.append('g').selectAll('.country').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'slate');
  svg.append('g').selectAll('.city').data(datapoints).enter().append('circle').attr('r', 0.75).attr('transform', function (d) {
    return "translate(".concat(projection([d.lng, d.lat]), ")");
  }).attr('fill', function (d) {
    return colorScale(d.population);
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/world.topojson":"data/world.topojson","/data/world-cities.csv":"data/world-cities.csv"}]},{},["scripts/fun-map.js"], null)
//# sourceMappingURL=/fun-map.a5ff09c3.js.map