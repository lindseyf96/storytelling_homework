...
module.exports = "/us_states.cc3a4cbd.topojson";
},{}],"data/powerplants.csv":[function(require,module,exports) {
module.exports = "/powerplants.f1ef6e44.csv";
},{}],"scripts/bubble-map.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#bubble-map').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var projection = d3.geoAlbersUsa();
var colorScale = d3.scaleOrdinal().range(['#ec913f', '#99979a', '#c1619c', '#2a83c2', '#de473a', '#51a74c', '#d7c54f', '#fdeed7']);
var radiusScale = d3.scaleSqrt().domain([0, 5000]).range([0, 15]); // out geoPath needs a PROJECTION variable
...
function ready(_ref) {
  ...
  svg.append('g').selectAll('.state').data(states.features).enter().append('path').attr('class', 'state').attr('d', path).attr('fill', '#e1e1e1');
  ...
  svg.append('g').selectAll('.state-label').data(states.features).enter().append('text').attr('class', 'state-label').attr('transform', function (d) {
    return "translate(".concat(path.centroid(d), ")");
  }).text(function (d) {
    return d.properties.abbrev;
  }).attr('text-anchor', 'middle').style('text-shadow', '-1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff');
  var legend = svg.append('g').attr('transform', 'translate(-140, 100)');
  var legendScale = d3.scaleBand().domain(colorScale.domain()).range([0, 400]);
  legend.selectAll('g').data(colorScale.domain()).enter().append('g').attr('transform', function (d) {
    return "translate(0,".concat(legendScale(d), ")");
  }).each(function (d) {
    var group = d3.select(this);
    group.append('circle').attr('r', 10).attr('fill', colorScale(d));
    group.append('text').text(d.charAt(0).toUpperCase() + d.slice(1)).attr('alignment-baseline', 'middle').attr('font-size', 12).attr('dx', 14);
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/us_states.topojson":"data/us_states.topojson","/data/powerplants.csv":"data/powerplants.csv"}]},{},["scripts/bubble-map.js"], null)
//# sourceMappingURL=/bubble-map.73b9e2a3.js.map