...
},{}],"scripts/03c-donut-wedges-multiples.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-3c').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec'];
var radius = 60;
var radiusScale = d3.scaleLinear().domain([0, 90]).range([0, radius]);
var cities = ['NYC', 'Tuscon', 'Lima', 'Beijing', 'Melbourne', 'Stockholm'];
...
var colorScale = d3.scaleLinear().range(['#B6D5E3', '#F4BFCB']).domain([35, 100]);
...
function ready(datapoints) {
  ...
  container.selectAll('.graph').data(nested).enter().append('g').attr('transform', function (d) {
    ...
    g.selectAll('path').data(d.values).enter().append('path').attr('d', function (d) {
      return arc(d);
    }).style('fill', function (d) {
      return colorScale(d.high_temp);
    });
    g.append('circle').attr('r', 2).attr('cx', 0).attr('cy', 0);
    g.append('text').text(function (d) {
      return d.key;
    }).attr('x', xPositionScale(d.cities)).attr('y', height / 3).attr('font-size', 15).attr('fill', 'black').attr('text-anchor', 'middle');
  });
}
},{"d3":"../node_modules/d3/index.js","/data/all-temps.csv":"data/all-temps.csv"}]},{},["scripts/03c-donut-wedges-multiples.js"], null)
//# sourceMappingURL=/03c-donut-wedges-multiples.10115b5d.js.map