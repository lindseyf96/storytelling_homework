...
var svg = d3.select('#chart-6').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
var category = ['Food', 'Service', 'Atmosphere', 'Price', 'Trendiness'];
var angleScale = d3.scaleBand().domain(category).range([0, Math.PI * 2]);
var radius = 150;
var radiusScale = d3.scaleLinear().domain([0, 5]).range([0, radius]);
var line = d3.radialLine().angle(function (d) {
  return angleScale(d.category);
}).radius(function (d) {
  return radiusScale(d.score);
});
d3.csv(require('/data/ratings.csv')).then(ready).catch(function (err) {
  return console.log('Failed with', err);
});

function ready(datapoints) {
  datapoints.push(datapoints[0]);
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', 'pink').attr('opacity', 0.4).attr('stroke', 'black');
  svg.append('circle').attr('r', 3).attr('cx', 0).attr('cy', 0).attr('fill', 'gray');
  var bands = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5];
  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', function (d) {
    console.log(d);
    return radiusScale(d);
  }).lower();
  svg.selectAll('.radius-line').data(angleScale.domain()).enter().append('line').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', -radius).attr('stroke', 'gray').attr('transform', function (d) {
    return "rotate(".concat(angleScale(d) * 180 / Math.PI, ")");
  });
  svg.selectAll('.outside-label').data(angleScale.domain()).enter().append('text').text(function (d) {
    return d;
...
},{"d3":"../node_modules/d3/index.js","/data/ratings.csv":"data/ratings.csv"}]},{},["scripts/06-radar.js"], null)
//# sourceMappingURL=/06-radar.cc29ccd9.js.map