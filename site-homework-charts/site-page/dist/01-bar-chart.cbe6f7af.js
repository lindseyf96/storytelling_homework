...
var yPositionScale = d3.scaleLinear().domain([0, 85]).range([height, 0]);
d3.csv(require('../data/countries.csv')).then(ready);

function ready(datapoints) {
  console.log('Data read in:', datapoints); // Sort the countries from low to high
  ...
  svg.selectAll('rect').data(datapoints).enter().append('rect').attr('width', xPositionScale.bandwidth()).attr('height', function (d) {
    return height - yPositionScale(d.life_expectancy);
  }).attr('fill', 'lightgrey').attr('x', function (d) {
...
},{"d3":"../node_modules/d3/index.js","../data/countries.csv":"data/countries.csv"}]},{},["scripts/01-bar-chart.js"], null)
