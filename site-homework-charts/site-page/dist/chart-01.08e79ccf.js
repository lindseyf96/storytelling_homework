...
var xPositionScale = d3.scaleLinear().domain([0, 70000]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 80]).range([height, 0]);
d3.csv(require('../data/countries.csv')).then(ready);

function ready(datapoints) {
  console.log('Data read in:', datapoints); // Go get the thing with an id of highlight-asia
  // listen for a 'click' event. when it's clicked,
  // run this function

  d3.select('#highlight-asia').on('click', function () {
    console.log('clicked'); // first, make ALL circles grey

    svg.selectAll('circle').attr('fill', 'lightgrey'); // then make asian circles red

    svg.selectAll('.asia').attr('fill', 'red');
  });
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 2).attr('class', function (d) {
...
},{"d3":"../node_modules/d3/index.js","../data/countries.csv":"data/countries.csv"}]},{},["scripts/chart-01.js"], null)
