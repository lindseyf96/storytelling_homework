...
var svg = d3.select('#slope-graph').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create your scales

var xPositionScale = d3.scalePoint().domain(['2015', '2016']).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 36]).range([height, 0]);
var colorScale = d3.scaleOrdinal().domain(['Maryland', 'Delaware', 'Maine', 'Florida', 'Illinois', 'Alaska', 'Arkansas', 'Colorado', 'Indiana', 'Iowa', 'Kentucky', 'Louisiana', 'Minnesota', 'Missouri', 'Nebraska', 'North Dakota', 'Texas', 'Virginia', 'Washington', 'Wyoming']).range(['#4cc1fc', '#4cc1fc', '#4cc1fc', '#4cc1fc', '#4cc1fc', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333', '#333333']);
var line = d3.line().x(function (d) {
  return xPositionScale(d.year);
}).y(function (d) {
  return yPositionScale(d.deaths_per_100k);
});
d3.csv(require('../data/overdoses.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 3).attr('cx', function (d) {
    return xPositionScale(d.year);
  }).attr('cy', function (d) {
    return yPositionScale(d.deaths_per_100k);
  ...
  }).attr('stroke-width', 1.5).attr('fill', 'none').attr('d', function (d) {
  ...
  svg.selectAll('text').data(nested).enter().append('text').attr('font-size', 12).attr('fill', '#333333').attr('x', xPositionScale('2016')).attr('dx', 5).attr('y', function (d) {
    ...
    d3.select(this).transition().duration(100);
    d3.selectAll('.' + this.getAttribute('class')).attr('fill', 'black');
    d3.selectAll('.' + this.getAttribute('class')).attr('stroke', 'black'); // d3.selectAll('text').attr('fill', function(d) {
    //   if (d.class == 'Maryland') {
    //     return 'blue'
    //   } else {
    //     return 'black'
  ...
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).lower();
  var yAxis = d3.axisLeft(yPositionScale).tickValues([5, 10, 15, 20, 25, 30, 35]).tickSize(-width);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis).lower();
  svg.selectAll('.y-axis path').remove();
  svg.selectAll('.y-axis text').remove();
  svg.selectAll('.y-axis line').attr('stroke-dasharray', 2).attr('stroke', 'grey');
}
},{"d3":"../node_modules/d3/index.js","../data/overdoses.csv":"data/overdoses.csv"}]},{},["scripts/03-slope-graph.js"], null)
//# sourceMappingURL=/03-slope-graph.e6cb6087.js.map